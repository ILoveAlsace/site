(() => {
  ...
  function loadKeBookingWidgets() {
    ...
    document.querySelectorAll('[data-ke-widget="engine"]').forEach(container => {
      if (container.dataset.loaded === widgetLang) return;
      container.dataset.loaded = widgetLang;
      container.innerHTML = '<div id="ke-booking-ibe-frame" style="width:100%;height:100%"></div>';
      const frame = container.querySelector("#ke-booking-ibe-frame");
      const script = document.createElement("script");
      script.src = `https://widgets.ke-booking.com/${widgetLang}/booking/engine/v3/js/${KE_BOOKING.token}/ke-booking.js`;
      script.type = "text/javascript";
      frame.appendChild(script);
    });
  }
    ...
    app.innerHTML = `
      <section class="hero">
        <div class="hero-content reveal">
          ...
          <div class="hero-actions">
            <a class="button" href="#/booking">${esc(t("actions.book"))}</a>
      ...
      <section class="section compact direct-booking-section">
        <div class="section-head reveal">
          ...
          <div><p class="lead">${esc(t("booking.text"))}</p>${contactLinks("inline-contact-links")}</div>
        </div>
    ...
    dialogContent.innerHTML = `
      ...
      <div class="dialog-body">
        ...
        <div class="dialog-booking-actions">
          <a class="button" href="${unitBookingUrl(unit)}" ${unitBookingUrl(unit).startsWith("http") ? 'target="_blank" rel="noopener noreferrer"' : ""}>${esc(t("actions.bookBooking"))}</a>
          <a href="${CONTACT.phoneHref}">${CONTACT.phoneDisplay}</a>
          <a href="mailto:${CONTACT.email}">${CONTACT.email}</a>
        </div>
      </div>`;
        ...
        <div class="contact-grid">
          ...
          <article class="contact-card reveal">
            ...
            <div class="contact-property-list">
              ${propertyList().map(p => `<a class="contact-property-link" href="#/property/${p.id}"><span>${esc(propertyName(p))}</span><span>${esc(t("actions.viewProperty"))} →</span></a>`).join("")}
            </div>
          </article>
        </div>
      </section>
  ...
  function renderBooking() {
    app.innerHTML = `
      ...
      <section class="section compact booking-engine-section">
        <div class="booking-engine-shell reveal" data-ke-widget="engine"></div>
        <aside class="booking-help-card reveal">
          <h2>${esc(t("booking.help"))}</h2>
          ${contactLinks("large-contact-links compact")}
          <p><a href="${propertyBookingUrl()}" target="_blank" rel="noopener noreferrer">${esc(t("booking.open"))} ↗</a></p>
        </aside>
      </section>`;
  ...
  function route() {
    ...
    requestAnimationFrame(() => { initReveals(); loadKeBookingWidgets(); });
  }
