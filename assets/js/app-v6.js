(() => {
  ...
  function unitMedia(unit, className = "") {
    ...
    return `<span class="unit-photo-placeholder ${className}" role="img" aria-label="${esc(t("property.photosPending"))}">
      <img src="assets/images/maison-champignon-logo.png" alt="" aria-hidden="true">
      <strong>${esc(unitName(unit))}</strong>
      <small>${esc(t("property.photosPending"))}</small>
    </span>`;
  }
  ...
  function route() {
    ...
    requestAnimationFrame(() => { initReveals(); loadKeBookingWidgets(); bindUnitDetailButtons(); });
  }
