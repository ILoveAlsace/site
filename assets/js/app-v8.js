  ...
  function visualJourneyMarkup(mode = "compact") {
    ...
    return `<div class="visual-journey visual-journey-${mode}" data-visual-journey data-items="${serialized}">
      <div class="visual-puzzle" aria-label="${esc(t("visual.instruction"))}">
        ${initial.map((item, index) => `<a class="visual-tile visual-tile-${index + 1}" href="${item.bookingUrl}" target="_blank" rel="noopener noreferrer" data-visual-tile data-unit-id="${item.unit.id}" aria-label="${esc(t("visual.book"))} — ${esc(unitName(item.unit))}">
